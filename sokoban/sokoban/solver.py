...
def breadthFirstSearch(gameState):
    ...
    # Chọn 1 node từ frontier để mở rộng cho đến khi frontier mở rộng
    while frontier:
        # Lấy 1 node từ đầu hàng đợi (các node ở đầu hàng đợi đều nông nhất)
        node = frontier.popleft()
        node_action = actions.popleft()

        # Kiểm tra xem node đang xét có phải trạng thái kết thúc hay không:
        # - nếu là trạng thái kết thúc thì lưu chuỗi hành động đến được trạng thái kết thúc vào temp 
        # và thoát khỏi vòng lặp
        if isEndState(node[-1][-1]):
            temp += node_action[1:]
            break

        # Kiểm tra xem node đang xét có thuộc tập đóng (exploredSet) hay chưa:
        # - nếu thuộc tức là đã xét trước đó nên không cần mở rộng node này
        # - ngược lại thì mở rộng node này
        if node[-1] not in exploredSet:
            ...
            for action in legalActions(node[-1][0], node[-1][1]):
                ...
                # Thêm vị trí mới của Player và các box cùng hành động tương ứng vào hàng đợi
...
def uniformCostSearch(gameState):
    ...
    # Chọn 1 node từ frontier để mở rộng cho đến khi frontier mở rộng
    while frontier:
        # Lấy 1 node từ đầu hàng đợi ưu tiên (các node ở đầu hàng đợi đều có cost nhỏ nhất)
        node = frontier.pop()
        node_action = actions.pop()
